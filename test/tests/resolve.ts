...
describe('resolve cases', () => {
    ...
    it('should create a resolved promise with a compound value and get the value', () => {
        ...
        return ZalgoPromise.resolve(value1)
            .then((result1) => {
                return [ result1, value2 ];
            })
            .then(
                // @ts-ignore - arguments in then are unexpected
                ([ result1, result2 ]) => {
                    if (result1 !== value1) {
    ...
    it('should create a resolved promise and register multiple then handlers', () => {
        ...
        return ZalgoPromise.all([
            ...
            .then(() => {
                if (thenCount !== 2) {
                    throw new Error(
                        `Expected then to have been called 2 times, got ${ thenCount } calls`
                    );
                }
            })
            .toPromise();
    ...
    it('should create a resolved promise and register multiple then handlers with one failure', () => {
        ...
        return ZalgoPromise.all([
            ...
            .catch(() => {
                ...
                if (thenCount !== 3) {
                    throw new Error(
                        `Expected then to have been called 3 times, got ${ thenCount } calls`
                    );
                }
            })
            .then(() => {
